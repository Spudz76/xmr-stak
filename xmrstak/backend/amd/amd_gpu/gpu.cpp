...
size_t InitOpenCLGpu(cl_context opencl_ctx, GpuContext* ctx, const char* source_code)
{
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when querying a device's max worksize using clGetDeviceInfo.", err_to_str(ret));
		return ERR_OCL_API;
	}
	 ...
	 */
	MaximumWorkSize /= 8;
	printer::inst()->print_backend_msg("OpenCL", L1,"Device %lu work size %lu / %lu.", ctx->deviceIdx, ctx->workSize, MaximumWorkSize);
#if defined(CL_VERSION_2_0) && !defined(CONF_ENFORCE_OpenCL_1_2)
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateCommandQueueWithProperties.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateBuffer to create input buffer.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateBuffer to create hash scratchpads buffer.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateBuffer to create hash states buffer.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateBuffer to create Branch 0 buffer.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateBuffer to create Branch 1 buffer.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateBuffer to create Branch 2 buffer.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateBuffer to create Branch 3 buffer.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateBuffer to create output buffer.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetDeviceInfo to get CL_DEVICE_NAME for device %u.", err_to_str(ret),ctx->deviceIdx );
		return ERR_OCL_API;
	}
	...
	for(int ii = 0; ii < num_algos; ++ii)
	{
		...
		if(xmrstak::params::inst().AMDCache == false || !clBinFile.good())
		{
			if(xmrstak::params::inst().AMDCache)
				printer::inst()->print_backend_msg("OpenCL", L1,"device %u - Precompiled code %s not found. Compiling ...",ctx->deviceIdx, cache_file.c_str());
			ctx->Program[ii] = clCreateProgramWithSource(opencl_ctx, 1, (const char**)&source_code, NULL, &ret);
			if(ret != CL_SUCCESS)
			{
				printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateProgramWithSource on the OpenCL miner code", err_to_str(ret));
				return ERR_OCL_API;
			}
			...
			{
				size_t len;
				printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clBuildProgram.", err_to_str(ret));

				if((ret = clGetProgramBuildInfo(ctx->Program[ii], ctx->DeviceID, CL_PROGRAM_BUILD_LOG, 0, NULL, &len)) != CL_SUCCESS)
				{
					printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clGetProgramBuildInfo for length of build log output.", err_to_str(ret));
					return ERR_OCL_API;
				}
				...
				{
					free(BuildLog);
					printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clGetProgramBuildInfo for build log.", err_to_str(ret));
					return ERR_OCL_API;
				}
			...
			do
			{
				...
				{
					printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clGetProgramBuildInfo for status of build.", err_to_str(ret));
					return ERR_OCL_API;
				}
				port_sleep(1);
			}
			...
			if(xmrstak::params::inst().AMDCache)
			{
				...
				{
					printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clGetProgramInfo.", err_to_str(ret));
					return ERR_OCL_API;
				}
				...
				file_stream.close();
				printer::inst()->print_backend_msg("OpenCL", L1, "device %u - Precompiled code stored in file %s",ctx->deviceIdx, cache_file.c_str());
			}
		}
		else
		{
			printer::inst()->print_backend_msg("OpenCL", L1, "device %u - Load precompiled code from file %s",ctx->deviceIdx, cache_file.c_str());
			...
			if(ret != CL_SUCCESS)
			{
				printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateProgramWithBinary. Try to delete file %s", err_to_str(ret), cache_file.c_str());
				return ERR_OCL_API;
			}
			ret = clBuildProgram(ctx->Program[ii], 1, &ctx->DeviceID, NULL, NULL, NULL);
			if(ret != CL_SUCCESS)
			{
				printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clBuildProgram. Try to delete file %s", err_to_str(ret), cache_file.c_str());
				return ERR_OCL_API;
			}
		}
		...
		if(ii == 0)
		{
			for(int i = 0; i < 7; ++i)
			{
				...
				if(ret != CL_SUCCESS)
				{
					printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateKernel for kernel_0 %s.", err_to_str(ret), KernelNames[i].c_str());
					return ERR_OCL_API;
				}
			}
		}
		else
		{
			for(int i = 0; i < 3; ++i)
			{
				...
				if(ret != CL_SUCCESS)
				{
					printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateKernel for kernel_1 %s.", err_to_str(ret), KernelNames[i].c_str());
					return ERR_OCL_API;
				}
			}
...
uint32_t getNumPlatforms()
{
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetPlatformIDs for number of platforms.", err_to_str(clStatus));
		return 0u;
	}
...
std::vector<GpuContext> getAMDDevices(int index)
{
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetPlatformIDs for platform information.", err_to_str(clStatus));
		return ctxVec;
	}

	if((clStatus = clGetDeviceIDs( platforms[index], CL_DEVICE_TYPE_GPU, 0, NULL, &num_devices)) != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetDeviceIDs for of devices.", err_to_str(clStatus));
		return ctxVec;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetDeviceIDs for device information.", err_to_str(clStatus));
		return ctxVec;
	}
	...
	for (size_t k = 0; k < num_devices; k++)
	{
		...
		{
			printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetDeviceInfo to get the device vendor name for device %u.", err_to_str(clStatus), k);
			continue;
		}
		...
		if((isAMDDevice && selectedOpenCLVendor == "AMD") || (isNVIDIADevice && selectedOpenCLVendor == "NVIDIA"))
		{
			...
			{
				printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetDeviceInfo to get CL_DEVICE_MAX_COMPUTE_UNITS for device %u.", err_to_str(clStatus), k);
				continue;
			}

			if((clStatus = clGetDeviceInfo(device_list[k], CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(size_t), &(maxMem), NULL)) != CL_SUCCESS)
			{
				printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetDeviceInfo to get CL_DEVICE_MAX_MEM_ALLOC_SIZE for device %u.", err_to_str(clStatus), k);
				continue;
			}

			if((clStatus = clGetDeviceInfo(device_list[k], CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(size_t), &(ctx.freeMem), NULL)) != CL_SUCCESS)
			{
				printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetDeviceInfo to get CL_DEVICE_GLOBAL_MEM_SIZE for device %u.", err_to_str(clStatus), k);
				continue;
			}
			...
			{
				printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: %s when calling clGetDeviceInfo to get CL_DEVICE_NAME for device %u.", err_to_str(clStatus), k);
				continue;
			}
			...
			printer::inst()->print_backend_msg("OpenCL", L0,"Found GPU %s.",ctx.name.c_str());
...
int getAMDPlatformIdx()
{
	...
	if(numPlatforms == 0)
	{
		printer::inst()->print_backend_msg("OpenCL", L0,"WARNING: No platform found.");
		return -1;
	}
	cl_platform_id * platforms = NULL;
	cl_int clStatus;
	...
	if(clStatus == CL_SUCCESS)
	{
		for (int i = 0; i < numPlatforms; i++) {
			...
			{
				printer::inst()->print_backend_msg("OpenCL", L0,"Found %s platform index id = %i, name = %s", selectedOpenCLVendor.c_str(), i , platformName.c_str());
		...
		{
			printer::inst()->print_backend_msg("OpenCL", L0,"No AMD platform found select Mesa as OpenCL platform");
...
size_t InitOpenCL(GpuContext* ctx, size_t num_gpus, size_t platform_idx)
{
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clGetPlatformIDs for number of platforms.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Selected OpenCL platform index %d doesn't exist.", platform_idx);
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clGetPlatformIDs for platform ID information.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"WARNING: using non AMD device: %s", platformName.c_str());
	}

	if((ret = clGetDeviceIDs(PlatformIDList[platform_idx], CL_DEVICE_TYPE_GPU, 0, NULL, &entries)) != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clGetDeviceIDs for number of devices.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	for(int i = 0; i < num_gpus; ++i)
	{
		...
		{
			printer::inst()->print_backend_msg("OpenCL", L1,"Selected device index %lu doesn't exist.\n", ctx[i].deviceIdx);
			return ERR_STUPID_PARAMS;
		}
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clGetDeviceIDs for device ID information.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	if(ret != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clCreateContext.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	for(int i = 0; i < num_gpus; ++i)
	{
		if(ctx[i].stridedIndex == 2 && (ctx[i].rawIntensity % ctx[i].workSize) != 0)
		{
			...
			printer::inst()->print_backend_msg("OpenCL", L0, "WARNING %s: gpu %d intensity is not a multiple of 'worksize', auto reduce intensity to %d", backendName.c_str(), ctx[i].deviceIdx, int(reduced_intensity));
		}
...
size_t XMRSetJob(GpuContext* ctx, uint8_t* input, size_t input_len, uint64_t target, xmrstak_algo miner_algo)
{
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clEnqueueWriteBuffer to fill input buffer.", err_to_str(ret));
		return ERR_OCL_API;
	}

	if((ret = clSetKernelArg(ctx->Kernels[kernel_storage][0], 0, sizeof(cl_mem), &ctx->InputBuffer)) != CL_SUCCESS)
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 0, argument 0.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 0, argument 1.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 0, argument 2.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 0, argument 3.", err_to_str(ret));
		return(ERR_OCL_API);
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 1, argument 0.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 1, argument 1.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 1, argument 2.", err_to_str(ret));
		return(ERR_OCL_API);
	}
	...
	if(miner_algo == cryptonight_monero || miner_algo == cryptonight_aeon || miner_algo == cryptonight_ipbc || miner_algo == cryptonight_stellite || miner_algo == cryptonight_masari || miner_algo == cryptonight_bittube2)
	{
		...
		{
			printer::inst()->print_backend_msg("OpenCL", L1, "Error %s when calling clSetKernelArg for kernel 1, argument 4(input buffer).", err_to_str(ret));
			return ERR_OCL_API;
		}
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 2, argument 0.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 2, argument 1.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 2, argument 2.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 2, argument 3.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 2, argument 4.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 2, argument 5.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel 2, argument 6.", err_to_str(ret));
		return(ERR_OCL_API);
	}
	...
	for(int i = 0; i < 4; ++i)
	{
		...
		{
			printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel %d, argument %d.", err_to_str(ret), i + 3, 0);
			return ERR_OCL_API;
		}
		...
		{
			printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel %d, argument %d.", err_to_str(ret), i + 3, 1);
			return ERR_OCL_API;
		}
		...
		{
			printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel %d, argument %d.", err_to_str(ret), i + 3, 2);
			return ERR_OCL_API;
		}
		...
		{
			printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel %d, argument %d.", err_to_str(ret), i + 3, 3);
			return ERR_OCL_API;
		}
	}
...
size_t XMRRunJob(GpuContext* ctx, cl_uint* HashOutput, xmrstak_algo miner_algo)
{
	...
	for(int i = 2; i < 6; ++i)
	{
		...
		{
			printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clEnqueueWriteBuffer to zero branch buffer counter %d.", err_to_str(ret), i - 2);
			return ERR_OCL_API;
		}
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clEnqueueReadBuffer to fetch results.", err_to_str(ret));
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clEnqueueNDRangeKernel for kernel %d.", err_to_str(ret), 0);
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clEnqueueNDRangeKernel for kernel %d.", err_to_str(ret), 1);
		return ERR_OCL_API;
	}
	...
	{
		printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clEnqueueNDRangeKernel for kernel %d.", err_to_str(ret), 2);
		return ERR_OCL_API;
	}
	...
	for(int i = 0; i < 4; ++i)
	{
		if(BranchNonces[i])
		{
			...
			{
				printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clSetKernelArg for kernel %d, argument %d.", err_to_str(ret), i + 3, 4);
				return(ERR_OCL_API);
			}
			...
			{
				printer::inst()->print_backend_msg("OpenCL", L1,"Error %s when calling clEnqueueNDRangeKernel for kernel %d.", err_to_str(ret), i + 3);
				return ERR_OCL_API;
			}
		}
	}
